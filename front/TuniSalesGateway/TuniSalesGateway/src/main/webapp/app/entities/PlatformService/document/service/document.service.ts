...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class DocumentService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/documents', 'platformservice');
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...document,
      createdAt: document.createdAt?.toJSON() ?? null,
      updatedAt: document.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...restDocument,
      createdAt: restDocument.createdAt ? dayjs(restDocument.createdAt) : undefined,
      updatedAt: restDocument.updatedAt ? dayjs(restDocument.updatedAt) : undefined,
    };
  }
