...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'issueDate' | 'dueDate' | 'paidAt' | 'createdAt' | 'updatedAt'> & {
...
export class InvoiceService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/invoices', 'businessservice');
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...
      paidAt: invoice.paidAt?.toJSON() ?? null,
      createdAt: invoice.createdAt?.toJSON() ?? null,
      updatedAt: invoice.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...
      paidAt: restInvoice.paidAt ? dayjs(restInvoice.paidAt) : undefined,
      createdAt: restInvoice.createdAt ? dayjs(restInvoice.createdAt) : undefined,
      updatedAt: restInvoice.updatedAt ? dayjs(restInvoice.updatedAt) : undefined,
    };
  }
