...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'dueDate' | 'submittedAt' | 'validatedAt' | 'createdAt' | 'updatedAt'> & {
  dueDate?: string | null;
...
export class OrderService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/orders', 'businessservice');
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      dueDate: order.dueDate?.toJSON() ?? null,
      submittedAt: order.submittedAt?.toJSON() ?? null,
      validatedAt: order.validatedAt?.toJSON() ?? null,
      createdAt: order.createdAt?.toJSON() ?? null,
      updatedAt: order.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      dueDate: restOrder.dueDate ? dayjs(restOrder.dueDate) : undefined,
      submittedAt: restOrder.submittedAt ? dayjs(restOrder.submittedAt) : undefined,
      validatedAt: restOrder.validatedAt ? dayjs(restOrder.validatedAt) : undefined,
      createdAt: restOrder.createdAt ? dayjs(restOrder.createdAt) : undefined,
      updatedAt: restOrder.updatedAt ? dayjs(restOrder.updatedAt) : undefined,
    };
  }
