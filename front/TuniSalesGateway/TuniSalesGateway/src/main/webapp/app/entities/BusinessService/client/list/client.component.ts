...
export class ClientComponent implements OnInit {
  ...
  getStatusBadgeClass(status: string | null | undefined): string {
    const map: Record<string, string> = {
      ACTIVE: 'tsg-badge--active',
      SUSPENDED: 'tsg-badge--suspended',
      BLACKLISTED: 'tsg-badge--blacklisted',
    };
    return map[status || ''] || 'tsg-badge--neutral';
  }

  getCreditPercentage(client: IClient): number {
    if (!client.creditLimit || client.creditLimit === 0) {
      return 0;
    }
    return Math.min(((client.creditUsed || 0) / client.creditLimit) * 100, 100);
  }

  getCreditBarClass(client: IClient): string {
    const pct = this.getCreditPercentage(client);
    if (pct >= 90) {
      return 'tsg-credit-bar__fill--danger';
    }
    if (pct >= 70) {
      return 'tsg-credit-bar__fill--warning';
    }
