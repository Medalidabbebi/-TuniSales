...
export class VisitComponent implements OnInit {
  ...
  countVisitsByStatus(status: string): number {
    return this.visits?.filter(visit => visit.status === status).length ?? 0;
  }
  ...
  getVisitAvatar(visit: IVisit): string {
    const source = visit.client?.name || visit.mission?.title || visit.id?.toString() || '?';
