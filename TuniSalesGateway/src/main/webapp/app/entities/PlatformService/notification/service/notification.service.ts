...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'readAt' | 'createdAt'> & {
  readAt?: string | null;
...
export class NotificationService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/notifications', 'platformservice');
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      readAt: notification.readAt?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      readAt: restNotification.readAt ? dayjs(restNotification.readAt) : undefined,
